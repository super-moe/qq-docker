...
USER user
RUN ["/bin/bash", "-c", "wget -qO- -t1 -T2 \"https://api.github.com/repos/super-moe/linuxqq/releases/latest\" | grep \"browser_download_url\" | sed -r -n 's/.*\"browser_download_url\": *\"(.*)\".*/\\1/p' | xargs wget -O ~/linuxqq.tar.gz"]
